...
final class CreangerAuthTheme {
    ...
    /**
     * Google Sign-In button colours, following the official Google button spec:
     * white / #747775 border / #1F1F1F text in light mode,
     * #131314 / #8E918F border / #E3E3E3 text in dark mode.
     */
    static int googleButtonBg() {
        return isDark() ? 0xFF131314 : 0xFFFFFFFF;
    }

    static int googleButtonBorder() {
        return isDark() ? 0xFF8E918F : 0xFF747775;
    }

    static int googleButtonText() {
        return isDark() ? 0xFFE3E3E3 : 0xFF1F1F1F;
    }
    ...
    static final class FieldWithIconCompat {
        ...
        void refreshTheme() {
            ...
            field.setCursorColor(errorState ? errorColor() : accent());
            field.setHandlesColor(errorState ? errorColor() : accent());
            field.setHighlightColor(Theme.multAlpha(errorState ? errorColor() : accent(), 0.30f));
            if (errorState) {
    ...
    static FieldWithIconCompat iconFieldCompat(Context c, int iconRes, int hintRes, int inputType, int ime, boolean isOtp) {
        ...
        field.setHandlesColor(accent());
        field.setHighlightColor(Theme.multAlpha(accent(), 0.30f));
        if (isOtp) {
    ...
    static AuthFooter authFooter(Context c, Runnable onPrivacy, Runnable onTerms) {
        ...
        privacy.setPadding(dp(2), dp(4), dp(2), dp(4));
        ...
        terms.setPadding(dp(2), dp(4), dp(2), dp(4));
