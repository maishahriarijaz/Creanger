...
public final class CreangerAuthEngine {
     ...
     * sets the Creanger password + names via PUT /auth/v1/user and claims the
     * username on the own profiles row (RLS-scoped PATCH), recording agreement
     * acceptance. Requires the active Google session; without it there is no
     * account to complete.
     *
     * @param termsAcceptedAtMillis UTC millis when the agreement box was checked.
     */
    public AuthSession googleAuthComplete(String username, String fullName, @Nullable String displayName,
                                          @Nullable String password, long termsAcceptedAtMillis)
            throws IOException, CreangerApiException {
        ...
        api.updateOwnProfile(current.accessToken, current.user.id, username, fullName,
                displayName, toIso8601Utc(termsAcceptedAtMillis));

        CreangerUser merged = mergeProfile(
                updatedRemote != null ? updatedRemote : current.user,
                username, fullName, null, displayName);
    ...
    /** Formats UTC millis as the ISO-8601 timestamp PostgREST expects. */
    static String toIso8601Utc(long millis) {
    ...
    /**
     * Reads another user's public profile row (RLS permits reading all
     * profiles). Used to resolve peer display names for direct chats without
     * any new backend surface; returns {@code null} when the row is missing.
     */
    @Nullable
    public SupabaseAuthClient.ProfileRow getPeerProfile(String userId)
            throws IOException, CreangerApiException {
        ...
        try {
            ...
            synchronized (lock) {
                if (this.session != null && refreshToken.equals(this.session.refreshToken)
                        && isDefinitiveTokenFailure(e)) {
                    // The server rejected the refresh token itself: the whole
                    // session family is dead, clear the persisted session.
                    clearLocalSession();
                } else if (state == AuthState.REFRESHING) {
                    // Transient failure (network/timeout/5xx/rate-limit): the
                    // stored refresh token is still valid server-side, so keep
                    // the session and let the next call retry the rotation.
                    // Never wipe credentials on a network blip.
                    state = AuthState.AUTHENTICATED;
                }
            }
            throw e;
    ...
    /**
     * True only when the server definitively rejected the refresh token
     * itself (rotation/reuse/expiry). Any transport-level failure or
     * non-token API error must preserve the stored session for a retry.
     */
    private static boolean isDefinitiveTokenFailure(Throwable e) {
        if (!(e instanceof CreangerApiException)) {
            return false;
        }
        CreangerApiException api = (CreangerApiException) e;
        if (api.statusCode == 401) {
            return true;
        }
        if (api.error == null || api.error.code == null) {
            return false;
        }
        return ApiError.SESSION_EXPIRED_OR_REVOKED.equals(api.error.code)
                || ApiError.TOKEN_INVALID.equals(api.error.code);
    }
