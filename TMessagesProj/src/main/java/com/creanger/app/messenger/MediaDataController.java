...
import com.creanger.app.SQLite.SQLiteCursor;
import com.creanger.app.SQLite.SQLiteDatabase;
import com.creanger.app.SQLite.SQLiteException;
import com.creanger.app.SQLite.SQLitePreparedStatement;
import com.creanger.app.messenger.ringtone.RingtoneDataStore;
import com.creanger.app.messenger.ringtone.RingtoneUploader;
...
import com.creanger.app.tgnet.TLMethod;
...
import com.creanger.app.ui.ActionBar.EmojiThemes;
...
import com.creanger.app.ui.Components.ChatThemeBottomSheet;
...
import com.creanger.app.ui.Components.StickersArchiveAlert;
...
import com.creanger.app.ui.Stories.StoriesStorage;
...
public class MediaDataController extends BaseController {
    ...
    public static String SHORTCUT_CATEGORY = "com.creanger.app.messenger.SHORTCUT_SHARE";
    ...
    /** Convert TLRPC sticker set to local model — presentation only, no RPC */
    public LocalModels.LocalStickerSet toLocalStickerSet(TLRPC.StickerSetCovered set) {
        if (set == null || set.set == null) return null;
        return new LocalModels.LocalStickerSet(String.valueOf(set.set.id), set.set.short_name != null ? set.set.short_name : "", set.set.title != null ? set.set.title : "", set.set.count);
    }

    public static LocalModels.LocalStickerSetType getStickerSetType(TLRPC.StickerSetCovered set) {
        if (set == null) return LocalModels.LocalStickerSetType.documentsOnly();
        if (set instanceof TLRPC.TL_stickerSetFullCovered) {
            return LocalModels.LocalStickerSetType.fullCovered();
        } else if (set instanceof TLRPC.TL_stickerSetNoCovered) {
            return LocalModels.LocalStickerSetType.noCovered();
        }
        return LocalModels.LocalStickerSetType.documentsOnly();
    }
    ...
    public void loadStickers(int type, boolean cache, boolean force, boolean scheduleIfLoading, Utilities.Callback<ArrayList<TLRPC.TL_messages_stickerSet>> onFinish) {
        // Creanger: mask stickers require Telegram MTProto; skip network load, keep local cache path.
        if (BuildConfig.USE_CREANGER_AUTH && type == TYPE_MASK && !cache) {
            if (onFinish != null) {
                onFinish.run(null);
            }
            return;
...
//                }
    ...
    public void loadDraftsIfNeed() {
        ...
        // Creanger: drafts use local persistence only (SharedPreferences "drafts" loaded in
        // constructor + CreangerChatApiClient draft API for Creanger chats). Skip Telegram
        // cloud/DC synchronization via TL_messages_getAllDrafts.
    ...
    public void saveDraft(long dialogId, long threadId, CharSequence message, ArrayList<TLRPC.MessageEntity> entities, TLRPC.Message replyToMessage, ChatActivity.ReplyQuote quote, TLRPC.SuggestedPost suggestedPost, long effectId, boolean noWebpage, boolean clean, TL_iv.RichMessage richMessage) {
        ...
        if (threadId == 0 || ChatObject.isForum(chat) || ChatObject.isMonoForum(chat)) {
            // Creanger: keep local draft persistence (SharedPreferences) + dialog sort/reload,
            // but skip Telegram DC cloud sync via TL_messages_saveDraft.
            if (!DialogObject.isEncryptedDialog(dialogId) && !BuildConfig.USE_CREANGER_AUTH) {
...
//            }
    ...
    public SearchStickersKey searchStickers(boolean emojis, String lang_code, String q, Utilities.Callback<ArrayList<TLRPC.Document>> whenDone, boolean next) {
        ...
        // Creanger: global Telegram sticker search via MTProto hidden; local sticker
        // functionality (LocalStickerStore / recents / favorites) remains untouched.
        if (BuildConfig.USE_CREANGER_AUTH) {
            whenDone.run(new ArrayList<>());
            return new SearchStickersKey(emojis, lang_code, q);
        }
