...
public class ImageLocation {
    ...
    public static ImageLocation getForPhoto(TLRPC.PhotoSize photoSize, TLRPC.Photo photo) {
        ...
        // Synthetic (Creanger) photos carry a plain HTTPS source URL on the size
        // instead of an MTProto location; route them through the existing
        // ImageLoader HTTP/cache path (no MTProto file fetching).
        String sourceUrl = photoSize.url;
        if (sourceUrl != null && sourceUrl.length() > 0) {
            return getForPath(sourceUrl);
        }
        int dc_id;
        if (photo.dc_id != 0) {
            dc_id = photo.dc_id;
            ...
            // No location and no URL: nothing loadable (missing/invalid source).
            return null;
        }
    ...
    public static ImageLocation getForUserOrChat(Object object, int type) {
        return getForUserOrChat(UserConfig.selectedAccount, object, type);
    }

    public static ImageLocation getForUserOrChat(int currentAccount, Object object, int type) {
    ...
    public static ImageLocation getForDocument(TLRPC.PhotoSize photoSize, TLRPC.Document document) {
        ...
        // Synthetic (Creanger) video posters carry a plain HTTPS/local source
        // URL on the size instead of an MTProto location; route them through
        // the existing ImageLoader HTTP/cache path (no MTProto file fetching).
        String sourceUrl = photoSize.url;
        if (sourceUrl != null && sourceUrl.length() > 0) {
            return getForPath(sourceUrl);
        }
