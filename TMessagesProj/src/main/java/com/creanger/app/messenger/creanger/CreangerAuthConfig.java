 ...
 */
public class CreangerAuthConfig implements TransportConfig {
    ...
    /** True when a Supabase anon key is configured (required by GoTrue/PostgREST). */
    public boolean hasSupabaseAnonKey() {
        String key = getSupabaseAnonKey();
        return key != null && !key.isEmpty();
    }

    /** True when the auth backend is reachable in principle (URL + anon key). */
    public boolean hasBackendConfig() {
        return hasBaseUrl() && hasSupabaseAnonKey();
    }
