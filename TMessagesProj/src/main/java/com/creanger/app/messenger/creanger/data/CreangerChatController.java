...
public final class CreangerChatController {
    ...
    private final MessageRealtimeClient.Listener realtimeListener = new MessageRealtimeClient.Listener() {
        ...
        public void onTypingChanged(String chatId, String userId, boolean isTyping) {
            // Typing indicator has no chat-screen binding yet; surfaced via realtime only.
        }
    ...
    private void reportNetworkOrAuthError(CreangerApiException apiError, Throwable ioError) {
        // Rate limits are transient, not auth failures — stay out of AUTH_ERROR
        // so the UI doesn't show a "session expired" state for a slow-down.
        if (apiError != null && apiError.error != null
                && com.creanger.app.messenger.creanger.model.ApiError.RATE_LIMITED.equals(apiError.error.code)) {
            setPhase(Phase.NETWORK_ERROR);
        } else if (apiError != null || !(ioError instanceof java.io.IOException)) {
