...
import static com.creanger.app.ui.LaunchActivity.getLastFragment;
...
import com.creanger.app.messenger.RichMessageLayout;
import com.creanger.app.messenger.MessageSuggestionParams;
...
import com.creanger.app.messenger.camera.CameraController;
import com.creanger.app.messenger.utils.DrawableUtils;
import com.creanger.app.messenger.utils.EphemeralMessagesHelper;
...
import com.creanger.app.ui.BasePermissionsActivity;
...
import com.creanger.app.ui.Components.chat.SendButtonBlockedByTypingView;
import com.creanger.app.ui.Components.chat.layouts.ChatActivitySideControlsButtonsLayout;
import com.creanger.app.ui.Components.inset.WindowInsetsInAppController;
...
import com.creanger.app.ui.GroupStickersActivity;
import com.creanger.app.ui.LaunchActivity;
import com.creanger.app.ui.LinkManager;
import com.creanger.app.ui.MessageSendPreview;
import com.creanger.app.ui.MultiContactsSelectorBottomSheet;
...
import com.creanger.app.ui.iv.BlockRow;
import com.creanger.app.ui.iv.RichEditor;
import com.creanger.app.ui.iv.RichHtml;
import com.creanger.app.ui.iv.RichMessageConvert;
    ...
    FactorAnimator.Target, Theme.Colorable
{
    ...
    private Runnable openKeyboardRunnable = new Runnable() {
        @Override
        public void run() {
            if (BaseFragment.hasSheets(parentFragment)) {
                return;
            }
    ...
    private void createSenderSelectView() {
        ...
        senderSelectView.setOnClickListener(v -> {
            ...
            if (delegate.getSendAsPeers() != null) {
                ...
                TLRPC.Peer defPeer;
                final TLRPC.Chat currentChat = MessagesController.getInstance(currentAccount).getChat(-dialog_id);
                ...
                senderSelectPopupWindow = new SenderSelectPopup(getContext(), parentFragment, controller, isChannel, defPeer, delegate.getSendAsPeers(), (recyclerView, senderView, peer) -> {
                    ...
                    AndroidUtilities.runOnUIThread(() -> {
                        ...
                        final float endScale = senderSelectView.getLayoutParams().width * 1.0f / (float) dp(SenderSelectPopup.AVATAR_SIZE_DP);
                        avatar.setPivotX(0);
                        avatar.setPivotY(0);
...
//        }
    ...
    private void showAiButton(boolean show_) {
        // AI Compose feature removed; button no longer exists.
    }
    ...
    private void showTopView(boolean animated, final boolean openKeyboard, boolean skipAwait) {
        ...
        boolean openKeyboardInternal = (recordedAudioPanel == null || recordedAudioPanel.getVisibility() != VISIBLE) && (!forceShowSendButton && replyingQuote == null || openKeyboard);
...
//        }
    ...
    public void setChatInfo(TLRPC.ChatFull chatInfo) {
        ...
        if (slowModeButton != null) {
            slowModeButton.setPremiumMode(false);
        }
    ...
    private void checkIsEphemeralMessage(boolean animated) {
        ...
        final boolean isEphemeralVisible = dialog_id < 0 && isChat && editingMessageObject == null && (
            EphemeralMessagesHelper.getInstance(currentAccount).isEphemeralCommand(text, null)
...
//        }
    ...
    public void doneEditingMessage() {
        ...
        if (!TextUtils.equals(message[0], editingMessageObject.messageText) || entities != null && !entities.isEmpty() || !editingMessageObject.messageOwner.entities.isEmpty() || editingMessageObject.messageOwner.media instanceof TLRPC.TL_messageMediaWebPage) {
            ...
            } else if (parentFragment instanceof ChatActivity && ((ChatActivity) parentFragment).isCreangerChat) {
                CharSequence newContent = editingMessageObject.editingMessage != null ? editingMessageObject.editingMessage : message[0];
                String messageId = editingMessageObject.messageOwner != null && editingMessageObject.messageOwner.params != null
                        ? editingMessageObject.messageOwner.params.get("creanger_uuid")
                        : null;
                if (!TextUtils.isEmpty(messageId)) {
                    ((ChatActivity) parentFragment).creangerEditTextMessage(messageId, newContent.toString());
                }
            } else {
    ...
    public boolean processSendingText(CharSequence text, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars) {
        if (parentFragment instanceof ChatActivity) {
            ...
            if (chatActivity.isCreangerChat && scheduleDate == 0 && payStars == 0) {
                return chatActivity.creangerProcessSendingText(text, notify);
            }
        }
        if (replyingQuote != null && parentFragment != null && replyingQuote.outdated) {
            parentFragment.showQuoteMessageUpdate();
    ...
    private void updateFieldRight(int attachVisible) {
        ...
        if (attachVisible == 1 || attachVisible == 2/* && layoutParams.rightMargin != dp(2)*/) {
            if (notifyButton != null && notifyButton.getVisibility() == VISIBLE || scheduledButton != null && scheduledButton.getTag() != null) {
...
//        }
    ...
    public void setCommand(MessageObject messageObject, String command, boolean longPress, boolean username) {
        ...
        if (longPress) {
            ...
            if (username && user != null && user.bot && !command.contains("@")) {
    ...
    public void updateGiftButton(boolean animated) {
        ...
        final boolean visible =
            ...
            (
                ...
                (userInfo.disallowed_stargifts == null || !userInfo.disallowed_stargifts.disallow_premium_gifts)
            ) &&
    ...
    protected void createEmojiView() {
        ...
        emojiView.setDelegate(new EmojiView.EmojiViewDelegate() {
            ...
            public void onGifSelected(View view, Object gif, String query, Object parent, boolean notify, int scheduleDate, int scheduleRepeatPeriod, MediaController.PhotoEntry entry, boolean invertMedia) {
                if (parentFragment instanceof ChatActivity && ((ChatActivity) parentFragment).isCreangerChat) {
                    ((ChatActivity) parentFragment).creangerBlockUnsupportedSend(0, 0, "GIFs are not supported in Creanger chats yet");
                    return;
                }
                if (replyingQuote != null && parentFragment != null && replyingQuote.outdated) {
                    parentFragment.showQuoteMessageUpdate();
    ...
    public void onStickerSelected(TLRPC.Document sticker, String query, Object parent, MessageObject.SendAnimationData sendAnimationData, boolean clearsInputField, boolean notify, int scheduleDate, int scheduleRepeatPeriod) {
        if (parentFragment instanceof ChatActivity && ((ChatActivity) parentFragment).isCreangerChat) {
            ((ChatActivity) parentFragment).creangerBlockUnsupportedSend(0, 0, "Stickers are not supported in Creanger chats yet");
            return;
        }
        if (replyingQuote != null && parentFragment != null && replyingQuote.outdated) {
            parentFragment.showQuoteMessageUpdate();
...
//            }
    ...
    public static class SendButton extends View implements ItemOptions.ScrimView {
        ...
        public void setStarsPrice(long price, int count, boolean animated) {
            ...
            messagesCount = count;
            if (price > 0) {
                priceText.setText(LocaleController.formatNumber(price * Math.max(1, messagesCount), ','), animated);
            } else {
