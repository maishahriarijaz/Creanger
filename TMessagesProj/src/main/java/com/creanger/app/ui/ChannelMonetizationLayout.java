...
import static com.creanger.app.messenger.AndroidUtilities.REPLACING_TAG_TYPE_LINK_NBSP;
import static com.creanger.app.messenger.AndroidUtilities.dp;
import static com.creanger.app.messenger.AndroidUtilities.makeBlurBitmap;
...
import static com.creanger.app.tgnet.ConnectionsManager.DEFAULT_DATACENTER_ID;
...
public class ChannelMonetizationLayout extends SizeNotifierFrameLayout implements NestedScrollingParent3 {
    ...
    public ChannelMonetizationLayout(
        ...
        final int proceedsInfoText = R.string.MonetizationProceedsTONInfo;
        final int proceedsInfoLink = R.string.MonetizationProceedsTONInfoLink;
        ...
        balanceButton.setOnClickListener(v -> {
            if (!v.isEnabled() || balanceButton.isLoading()) {
                return;
            }
...
//            if (stats_dc == -1) return;
    ...
    public void setupBalances(boolean ton, TLRPC.TL_starsRevenueStatus balances) {
        if (ton) {
            ...
            lastWithdrawalValue.contains1 = false;
            lifetimeValue.contains1 = false;
            balanceButton.setVisibility(View.GONE);
        }
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        if (chat != null && chat.creator) {
            if (tonRevenueAvailable) {
                ...
                items.add(UItem.asCheck(CHECK_SWITCHOFF, getString(R.string.MonetizationSwitchOff)).setChecked(switchOffValue));
...
//        } else {
    ...
    public class ChannelTransactionsView extends LinearLayout {
        ...
        public void reloadTransactions() {
            ...
            if (loadingTransactions[TON_TRANSACTIONS]) return;
            tonTransactions.clear();
            tonTransactionsLastOffset = "";
            loadingTransactions[TON_TRANSACTIONS] = false;
            loadTransactions(TON_TRANSACTIONS);
        ...
        public boolean hasTransactions() {
            return !tonTransactions.isEmpty();
        }
        public boolean hasTransactions(int type) {
            return type == TON_TRANSACTIONS && !tonTransactions.isEmpty();
        }
