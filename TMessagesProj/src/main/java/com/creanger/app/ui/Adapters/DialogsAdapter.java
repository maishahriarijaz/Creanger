...
package com.creanger.app.ui.Adapters;
...
import com.creanger.app.ui.Cells.ArchiveHintCell;
import com.creanger.app.ui.Cells.DialogCell;
import com.creanger.app.ui.Cells.DialogMeUrlCell;
import com.creanger.app.ui.Cells.DialogsEmptyCell;
import com.creanger.app.ui.Cells.DialogsHintCell;
import com.creanger.app.ui.Cells.DialogsRequestedEmptyCell;
...
import com.creanger.app.ui.Components.ListView.AdapterWithDiffUtils;
...
public class DialogsAdapter extends RecyclerListView.SelectionAdapter implements DialogCell.DialogCellDelegate {
    ...
    //            VIEW_TYPE_ARCHIVE = 9,
            ...
            VIEW_TYPE_STORIES = 17,
            VIEW_TYPE_ARCHIVE_FULLSCREEN = 18,
            VIEW_TYPE_GRAY_SECTION = 19,
            VIEW_TYPE_FORWARD_TO_STORIES_CELL = 20,
            VIEW_TYPE_HEADER_3 = 21,
            VIEW_TYPE_DIALOG_COMMUNITY = 22,
            VIEW_TYPE_CREANGER_CHAT = 23;
     ...
     * rebuilds the list. Called on the UI thread after a background refresh.
     */
    public void setCreangerChats(List<CreangerChat> chats) {
        setCreangerChats(chats, null);
    }

    /**
     * Same as {@link #setCreangerChats(List)} plus direct-chat peer identity
...
//        }
    ...
    private class ItemInternal extends AdapterWithDiffUtils.Item {
        ...
        public ItemInternal(int viewType, CreangerChat creangerChat) {
            super(viewType, false);
            this.creangerChat = creangerChat;
            long stable = CreangerDialogList.stableId(
                    creangerChat != null ? creangerChat.id : null);
            int currentId = dialogsStableIds.get(stable, -1);
            if (currentId >= 0) {
                stableId = currentId;
            } else {
                stableId = stableIdPointer++;
                dialogsStableIds.put(stable, stableId);
            }
        }
        ...
        public ItemInternal(int viewType, TLRPC.Dialog dialog) {
            ...
            if (dialog != null) {
                if (dialogsType == 7 || dialogsType == 8) {
                    ...
                    pinned = filter != null && filter.pinnedDialogs != null && filter.pinnedDialogs.indexOfKey(dialog.id) >= 0;
                } else {
    ...
    public Object getItem(int i) {
        ...
        if (item.title != null) {
            ...
            return item.dialog;
        } else if (item.creangerChat != null) {
            return item.creangerChat;
    ...
    public void moveDialogs(RecyclerListView recyclerView, int fromPosition, int toPosition) {
        ...
        if (dialogsType == 7 || dialogsType == 8) {
            MessagesController.DialogFilter filter = getCurrentFilter();
            if (filter != null && filter.pinnedDialogs != null) {
    ...
    private MessagesController.DialogFilter getCurrentFilter() {
        // Folder ecosystem removed - return null
        return null;
    }
    ...
    private void updateItemList() {
        ...
        if (!stopUpdate) {
            ...
            // Creanger section: appended after the Telegram rows so both live
            // in one list. Gated to the default tab / main folder /
            // non-selection flows by CreangerDialogList.
            if (communityId == 0 && CreangerDialogList.shouldShow(dialogsType, folderId, isOnlySelect)) {
                for (int k = 0; k < creangerChats.size(); k++) {
                    CreangerChat chat = creangerChats.get(k);
                    if (chat != null && chat.id != null) {
                        itemInternals.add(new ItemInternal(VIEW_TYPE_CREANGER_CHAT, chat));
                    }
                }
            }
            ...
            if (communityId == 0 && !forceShowEmptyCell && dialogsType != 7 && dialogsType != 8 && !MessagesController.getInstance(currentAccount).isDialogsEndReached(folderId)) {
                ...
                itemInternals.add(new ItemInternal(VIEW_TYPE_LAST_EMPTY));
            } else if (dialogsCount == 0 && creangerChats.isEmpty()) {
                isEmpty = true;
                if (requestPeerType != null) {
                    itemInternals.add(new ItemInternal(VIEW_TYPE_REQUIRED_EMPTY));
                } else {
    ...
    public int getItemHeight(int position) {
        if (itemInternals.get(position).viewType == VIEW_TYPE_CREANGER_CHAT) {
            return AndroidUtilities.dp(70) + 1;
        }
