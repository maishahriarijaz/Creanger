...
import com.creanger.app.ui.Components.Premium.boosts.cells.selector.SelectorCountryCell;
...
//        containerView.addView(sectionCell, LayoutHelper.createFrameMarginPx(LayoutHelper.MATCH_PARENT, 32, Gravity.TOP | Gravity.FILL_HORIZONTAL, backgroundPaddingLeft, 0, backgroundPaddingLeft, 0));
        ...
        recyclerListView.setOnItemClickListener((view, position, x, y) -> {
            if (view instanceof TextCell) {
                ...
                if (selectedIds.size() == 11 && (type == TYPE_USER || type == TYPE_CHANNEL)) {
                    selectedIds.remove(id);
            ...
            if (view instanceof SelectorCountryCell) {
                ...
                if (selectedIds.size() == 12 && type == TYPE_COUNTRY) {
                    selectedIds.remove(id);
                    showMaximumUsersToast();
                    return;
                }
    ...
    private void updateSection() {
        ...
        switch (type) {
            case TYPE_CHANNEL:
                text = LocaleController.formatPluralString("BoostingSelectUpToGroupChannelPlural", 10);
            ...
            case TYPE_USER:
                ...
                break;
            case TYPE_COUNTRY:
                text = LocaleController.formatPluralString("BoostingSelectUpToCountriesPlural", 11);
    ...
    private void showMaximumUsersToast() {
        ...
        switch (type) {
            case TYPE_CHANNEL:
                text = LocaleController.formatPluralString("BoostingSelectUpToWarningChannelsGroupsPlural", 10);
                break;
            case TYPE_USER:
                text = LocaleController.getString(R.string.BoostingSelectUpToWarningUsers);
                break;
            case TYPE_COUNTRY:
                text = LocaleController.formatPluralString("BoostingSelectUpToWarningCountriesPlural", 11);
                break;
        }
        if (selectedObjectsListener != null) {
