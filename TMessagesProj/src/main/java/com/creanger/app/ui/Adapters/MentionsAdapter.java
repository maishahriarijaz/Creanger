...
import com.creanger.app.ui.Cells.MentionCell;
    ...
    private void searchForContextBotResults(final boolean cache, final TLRPC.User user, final String query, final String offset) {
        ...
        RequestDelegate requestDelegate = (response, error) -> AndroidUtilities.runOnUIThread(() -> {
            ...
            if (response instanceof TLRPC.TL_messages_botResults) {
                ...
                if (added) {
                    notifyItemChanged(searchResultBotContext.size() - res.results.size() - 1);
...
//                            delegate.needChangePanelVisibility(false);
    ...
    public int getItemCountInternal() {
        ...
        if (stickers != null) {
            ...
            count += searchResultBotContext.size();
...
//            if (!searchResultHashtags.isEmpty()) position--;
        ...
        if (type == 4) {
            ...
            if (position >= 0 && position < searchResultBotContext.size()) {
                ((ContextLinkCell) holder.itemView).setLink(searchResultBotContext.get(position), foundContextBot, contextMedia, position != searchResultBotContext.size() - 1, false, "gif".equals(searchingContextUsername));
            }
        } else if (type == 6) {
