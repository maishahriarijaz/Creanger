...
import static com.creanger.app.messenger.MediaDataController.MEDIA_PHOTOVIDEO;
import static com.creanger.app.messenger.MediaDataController.MEDIA_POLL;
...
import com.creanger.app.messenger.SavedMessagesController;
...
import com.creanger.app.messenger.utils.tlutils.TlUtils;
...
import com.creanger.app.ui.Adapters.SearchAdapterHelper;
import com.creanger.app.ui.CalendarActivity;
...
import com.creanger.app.ui.Cells.ManageChatUserCell;
import com.creanger.app.ui.Cells.ProfileSearchCell;
import com.creanger.app.ui.Cells.SharedAudioCell;
import com.creanger.app.ui.Cells.SharedDocumentCell;
import com.creanger.app.ui.Cells.SharedLinkCell;
import com.creanger.app.ui.Cells.SharedMediaSectionCell;
import com.creanger.app.ui.Cells.SharedPhotoVideoCell;
import com.creanger.app.ui.Cells.SharedPhotoVideoCell2;
import com.creanger.app.ui.Cells.UserCell;
import com.creanger.app.ui.ChatActivity;
import com.creanger.app.ui.ChatActivityContainer;
...
import com.creanger.app.ui.ProfileActivity2;
import com.creanger.app.ui.ProfileStoriesCollectionTabs;
import com.creanger.app.ui.SelectStoriesBottomSheet;
...
import com.creanger.app.ui.Stories.ViewsForPeerStoriesRequester;
...
import com.creanger.app.ui.ThemeActivity;
import com.creanger.app.ui.TopicsFragment;
...
public class SharedMediaLayout extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, DialogCell.DialogCellDelegate {
    ...
    public float getPhotoVideoOptionsAlpha(float progress) {
        ...
        float alpha = 0;
        if (mediaPages[1] != null && (mediaPages[1].selectedType == TAB_PHOTOVIDEO || (mediaPages[1].selectedType == TAB_STORIES || isStoryAlbumPageType(mediaPages[1].selectedType)) && TextUtils.isEmpty(getStoriesHashtag()) || mediaPages[1].selectedType == TAB_ARCHIVED_STORIES || mediaPages[1].selectedType == TAB_SAVED_DIALOGS))
            alpha += progress;
        if (mediaPages[0] != null && (mediaPages[0].selectedType == TAB_PHOTOVIDEO || (mediaPages[0].selectedType == TAB_STORIES || isStoryAlbumPageType(mediaPages[0].selectedType)) && TextUtils.isEmpty(getStoriesHashtag()) || mediaPages[0].selectedType == TAB_ARCHIVED_STORIES || mediaPages[0].selectedType == TAB_SAVED_DIALOGS))
            alpha += 1f - progress;
        return alpha;
    }
    ...
    public SharedMediaLayout(Context context, long did, SharedMediaPreloader preloader, int commonGroupsCount, ArrayList<Integer> sortedUsers, TLRPC.ChatFull chatInfo, TLRPC.UserFull userInfo, int initialTab, int initialStoryAlbumId, BaseFragment parent, Delegate delegate, int viewType, Theme.ResourcesProvider resourcesProvider, BlurredBackgroundDrawableViewFactory iBlur3FactoryLiquidGlass) {
        ...
        int[] mediaCount;
        if (preloader != null) {
            mediaCount = preloader.getLastMediaCount();
            topicId = sharedMediaPreloader.topicId;
        } else {
            mediaCount = new int[]{-1, -1, -1, -1, -1, -1, -1, -1, -1};
            topicId = 0;
        }
...
//        return customTabs() ? 0 : (48 /*+ 42*/);
    ...
    private void changeMediaFilterType() {
        ...
        // Creanger safety: null preloader means no Telegram media source; show safe empty state.
        if (sharedMediaPreloader == null || sharedMediaPreloader.getSharedMediaData() == null) {
            return;
        }
        final int[] counts = sharedMediaPreloader.getLastMediaCount();
        if (counts == null || sharedMediaPreloader.getSharedMediaData().length == 0) {
            return;
        }
    ...
    private boolean fillMediaData(int type) {
        // Creanger safety: preloader may be null for Creanger-compatible empty states.
        // Do not introduce fake Telegram media loading; leave local sharedMediaData empty.
    ...
    public boolean isOptionsItemVisible() {
        ...
        return type == TAB_PHOTOVIDEO || isAnyStoryPageType(type) || type == TAB_SAVED_DIALOGS;
    }
        ...
        protected void dispatchDraw(Canvas canvas) {
            ...
            if (isThisListView() && getAdapter() == movingAdapter) {
                    ...
                    if (child instanceof SharedPhotoVideoCell2) {
                        ...
                        if (isChangeColumnsAnimation()) {
                            ...
                            if (currentColumn >= 0 && currentColumn < getAnimateToColumnsCount() && toIndex >= 0 && toIndex < animationSupportingSortedCells.size()) {
                                inAnimation = true;
                                float toScale = (animationSupportingSortedCells.get(toIndex).getMeasuredWidth() - dp(2)) / (float) (cell.getMeasuredWidth() - dp(2));
                ...
                if (isChangeColumnsAnimation() && !drawingViews.isEmpty()) {
                    ...
                    float sizeToScale = ((getMeasuredWidth() / (float) getColumnsCount()) - dp(2)) / ((getMeasuredWidth() / (float) getAnimateToColumnsCount()) - dp(2));
                    ...
                    float size1 = (float) ((Math.ceil((getMeasuredWidth() / (float) getAnimateToColumnsCount())) - dp(2)) * scaleSize + dp(2));
                    if (isStories()) {
                        size1 *= 1.25f;
                    }
                ...
                if (isChangeColumnsAnimation()) {
                    ...
                    float sizeToScale = ((getMeasuredWidth() / (float) getAnimateToColumnsCount()) - dp(2)) / ((getMeasuredWidth() / (float) getColumnsCount()) - dp(2));
                    float scaleSize = sizeToScale * getChangeColumnsProgress() + (1f - getChangeColumnsProgress());

                    float size1 = (float) ((Math.ceil((getMeasuredWidth() / (float) getColumnsCount())) - dp(2)) * scaleSize + dp(2));
                    if (isStories()) {
                        size1 *= 1.25f;
                    }
                    float fromSize = getMeasuredWidth() / (float) getColumnsCount();
