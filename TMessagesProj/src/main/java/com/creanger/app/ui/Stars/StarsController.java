...
import static com.creanger.app.messenger.MediaDataController.calcHash;
...
public class StarsController {
    ...
    public boolean canUseTon() {
        ...
        if (StarsNeededSheet.allowTopUp()) {
            return true;
        }
    ...
    public void buy(
        ...
        if (BuildVars.useInvoiceBilling() || !BillingController.getInstance().isReady()) {
            ...
            final JSONObject themeParams = makeThemeParams(getResourceProvider());
            if (themeParams != null) {
            ...
            ConnectionsManager.getInstance(currentAccount).sendRequest(req2, (res2, err2) -> AndroidUtilities.runOnUIThread(() -> {
                ...
                if (!(res2 instanceof TLRPC.TL_payments_paymentResult)) {
                    if (err2 != null && "BALANCE_TOO_LOW".equals(err2.text)) {
                        ...
                        final boolean[] purchased = new boolean[] { false };
                        StarsIntroActivity.StarsNeededSheet sheet = new StarsIntroActivity.StarsNeededSheet(context, resourcesProvider, stars, StarsIntroActivity.StarsNeededSheet.TYPE_SUBSCRIPTION_BUY, name, () -> {
                            purchased[0] = true;
                            buyPremiumGift(dialogId, option, text, whenDone);
                        }, 0);
                        sheet.setOnDismissListener(d -> {
...
//                    }
    ...
    private JSONObject makeThemeParams(Theme.ResourcesProvider resourcesProvider) {
        try {
