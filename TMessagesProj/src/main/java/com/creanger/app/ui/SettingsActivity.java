...
import com.creanger.app.ui.Components.TextHelper;
...
public class SettingsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate, ImageUpdater.ImageUpdaterDelegate, MainTabsActivity.TabFragmentDelegate, FactorAnimator.Target {
    ...
    public View createView(Context context) {
        ...
        if (BuildConfig.USE_CREANGER_AUTH && CreangerAuth.getInstance(ApplicationLoader.applicationContext).getState() == AuthState.AUTHENTICATED) {
            otherItem.addSubItem(3, R.drawable.msg_leave, getString(R.string.CreangerLogOut));
        }
...
//        fragmentView.addView(navigationBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 0, Gravity.BOTTOM | Gravity.FILL_HORIZONTAL));
    ...
    public void setInfo(TLRPC.User user) {
        ...
        // Creanger users often have no phone number — never render "+null".
        if (user != null && !TextUtils.isEmpty(user.phone) && !"null".equals(user.phone)) {
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        } else if (suggestions.contains("VALIDATE_PHONE_NUMBER") && getUserConfig().getCurrentUser() != null && !BuildConfig.USE_CREANGER_AUTH) {
            items.add(SuggestionCell.Factory.of(
                ...
                getString(R.string.CheckPhoneNumberNo), v -> {
                    // Creanger: Telegram phone-number change is unsupported; entry hidden.
                },
                getString(R.string.CheckPhoneNumberYes2), v -> {
                    getMessagesController().removeSuggestion(0, "VALIDATE_PHONE_NUMBER");
                }
            ));
            items.add(UItem.asShadow(null));
        } else if (suggestions.contains("VALIDATE_PASSWORD") && !BuildConfig.USE_CREANGER_AUTH) {
            items.add(SuggestionCell.Factory.of(
        ...
        if (accountNumbers.size() > 0 && !BuildConfig.USE_CREANGER_AUTH) {
            // Creanger: Telegram multi-account switcher hidden; Creanger uses single Google auth session.
        ...
        // Creanger: Telegram gifts entry hidden (no Creanger gift implementation).
        // Premium page (id 11 above) is intentionally preserved and remains reachable.
        if (!getMessagesController().premiumPurchaseBlocked() && !BuildConfig.USE_CREANGER_AUTH) {
    ...
    private void confirmCreangerLogout() {
        Activity activity = getParentActivity();
        if (activity == null || activity.isFinishing()) return;
        new AlertDialog.Builder(activity)
                .setTitle(getString(R.string.CreangerLogOut))
                .setMessage(getString(R.string.CreangerLogOutConfirm))
                .setPositiveButton(getString(R.string.CreangerLogOut), (dialog, which) -> performCreangerLogout())
                .setNegativeButton(getString(R.string.Cancel), null)
                .show();
    }

    private void performCreangerLogout() {
        Activity activity = getParentActivity();
        if (activity == null || activity.isFinishing()) return;
        final AlertDialog progressDialog = new AlertDialog(activity, 3);
        progressDialog.setCanCancel(false);
        progressDialog.show();
        final CreangerAuth creanger = CreangerAuth.getInstance(ApplicationLoader.applicationContext);
        creanger.getAsync().logout(new CreangerAuthAsync.AuthCallback<Void>() {
            @Override
            public void onSuccess(Void result) {
                AndroidUtilities.runOnUIThread(() -> finishCreangerLogout(progressDialog));
            }

            @Override
            public void onError(CreangerApiException error, Throwable ioError) {
                creanger.getAsync().clearLocalSession();
                AndroidUtilities.runOnUIThread(() -> finishCreangerLogout(progressDialog));
            }
        });
    }

    private void finishCreangerLogout(AlertDialog progressDialog) {
        if (progressDialog != null) {
            try {
                progressDialog.dismiss();
            } catch (Exception ignore) {}
        }
        CreangerAuth creanger = CreangerAuth.getInstance(ApplicationLoader.applicationContext);
        creanger.getMessageRepository().clear();
        creanger.getChatRepository().clear();
        creanger.getCurrentUserRepository().clear();
    ...
    private void onClick(UItem item, View view, int position, float x, float y) {
        ...
        if (item.instanceOf(AccountCell.Factory.class)) {
            // Creanger: multi-account switcher disabled; ignore unreachable action.
            if (BuildConfig.USE_CREANGER_AUTH) {
                return;
            }
        ...
        switch (item.id) {
            ...
            case 5:
                ...
                break;
            case 6:
                // Creanger: Telegram Data/Network usage hidden.
            ...
            case 11:
                ...
                break;
            case 16:
                // Creanger: Telegram gifts unsupported.
                if (BuildConfig.USE_CREANGER_AUTH) break;
