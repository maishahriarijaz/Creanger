-- ============================================================================
-- CREANGER SUPABASE SCHEMA - MIGRATION 029: MEDIA MESSAGE SEND
-- ============================================================================
-- send_media_message(): a SECURITY DEFINER, member-gated, idempotent RPC that
-- persists a media message and its attachment metadata ATOMICALLY.
--
-- The binary file bytes are never uploaded through PostgREST: the Android
-- client only ever carries METADATA (storage_provider/storage_key/mime_type/
-- dimensions/duration/...). The actual file is uploaded to the external
-- provider (ImageBB for images, Cloudinary for video/audio/document) by the
-- Creanger/Admin backend, which records the storage_key; this RPC then creates
-- the DB rows (messages + media + message_attachments) in one transaction.
--
-- Design mirrors the message RPC precedents:
--   - send_text_message (migration 022): idempotent via client_message_id;
--     a retry with the same (chat, sender, client_message_id) is DO NOTHING and
--     returns the ORIGINAL message id (first write wins; attachments are never
--     re-inserted / mutated on retry).
--   - edit_message / delete_message / add_reaction (022/025/027): SECURITY
--     DEFINER with explicit authorization resolved from the caller JWT
--     (current_user_id()), never caller-supplied.
--   - mark readable hard failures: a non-member, a bogus message_type, or a
--     malformed attachment payload raises an observable P0001 error — never a
--     silent no-op (the way RLS/PostgREST would filter a POST to zero rows).
--
-- Atomicity: everything (message row + every media row + every
-- message_attachments row) is written inside the single SECURITY DEFINER
-- function body, so any failure rolls the whole send back. chat_seq is still
-- assigned by the trigger_assign_chat_seq BEFORE INSERT trigger.
-- ============================================================================
...
BEGIN
    ...
    FOR v_att IN SELECT * FROM jsonb_array_elements(p_attachments) LOOP
        ...
        INSERT INTO public.media (
            ...
            mime_type, size_bytes, checksum, width, height, duration
        )
        VALUES (
            v_user_id,
